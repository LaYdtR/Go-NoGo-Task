...
let expName = 'gng_task';  // from the Builder filename that created this script
let expInfo = {
...
psychoJS.start({
  ...
  resources: [
    ...
    {'name': 'gng_conditions.csv.xlsx', 'path': 'gng_conditions.csv.xlsx'},
  ]
});
...
async function experimentInit() {
  ...
  stim_prac = new visual.Polygon({
    win: psychoJS.window, name: 'stim_prac', 
...
function trials_loopLoopBegin(trials_loopLoopScheduler, snapshot) {
  return async function() {
    ...
    trials_loop = new TrialHandler({
      ...
      extraInfo: expInfo, originPath: undefined,
      trialList: 'gng_conditions.csv.xlsx',
...
function practice_trialRoutineBegin(snapshot) {
  return async function () {
    ...
    // update component parameters for each repeat
    stim_prac.setFillColor(new util.Color(color));
    stim_prac.setSize([0.25, 0.25]);
...
function practice_trialRoutineEnd(snapshot) {
    return async function () {
      ...
      if (routineForceEnded) {
          ...
          practice_trialClock.add(1.500000);
      }
      // Routines running outside a loop should always advance the datafile row
      if (currentLoop === psychoJS.experiment) {
        psychoJS.experiment.nextEntry(snapshot);
...
function trialRoutineBegin(snapshot) {
    return async function () {
      ...
      // update component parameters for each repeat
      stim.setFillColor(new util.Color(color));
      stim.setSize([0.25, 0.25]);
